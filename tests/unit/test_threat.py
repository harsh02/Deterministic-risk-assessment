...
print(f"  │     Easy to exploit with minimal skill required")
print()
print(f"  ├─ 35% × Known Exploited:    0.35 × {f['KnownExploited']:.0f}    = {known_contribution:.3f}")
print(f"  │  └─ Rationale: {'Active exploitation in the wild' if f['KnownExploited'] == 1 else 'No known exploitation'}")
print(f"  │     {'Attackers have working exploits available' if f['KnownExploited'] == 1 else 'Theoretical threat only'}")
print()
...
print(f"  │     {'Very common attack vector' if f['Attack_Frequency'] > 0.7 else 'Moderate frequency'}")
print()
...
print(f"        Probability this vulnerability will be exploited in the wild")
print()
print(f"  TOTAL: {exploit_contribution:.3f} + {known_contribution:.3f} + {attack_contribution:.3f} + {epss_contribution:.3f} = {scores['likelihood']:.3f}")
print()
...
if scores.get('severity_mode') == 'with_cvss':
    ...
    print(f"  ├─ 70% × CVSS Base Score:    0.70 × {f['CVSS_BaseScore']:.1f}/10 = {cvss_contribution:.3f}")
    print(f"  │  └─ Rationale: CVSS Base Score is {f['CVSS_BaseScore']:.1f}/10")
    if f['CVSS_BaseScore'] >= 9.0:
        print(f"  │     CRITICAL vulnerability with severe impact")
    elif f['CVSS_BaseScore'] >= 7.0:
        print(f"  │     HIGH severity vulnerability")
    else:
        print(f"  │     MEDIUM severity vulnerability")
    print()
    print(f"  ├─ 20% × Asset Criticality:  0.20 × {scores['context_criticality']:.1f}   = {criticality_contribution:.3f}")
    print(f"  │  └─ Rationale: Asset type is '{threat_input.get('context', {}).get('asset_type', 'Unknown')}'")
    print(f"  │     Business criticality: {scores['context_criticality']*100:.0f}%")
    print()
    print(f"  └─ 10% × Impact Scope:       0.10 × {f.get('Impact_Scope', 0.5):.2f}  = {scope_contribution:.3f}")
    ...
    print()
    print(f"  TOTAL: {cvss_contribution:.3f} + {criticality_contribution:.3f} + {scope_contribution:.3f} = {scores['severity']:.3f}")
else:
    # Threat path (no CVSS)
    ...
    if impact_val >= 0.9:
        print(f"  │     Data destruction or ransomware-level impact")
    elif impact_val >= 0.7:
        print(f"  │     Service disruption or data manipulation")
    else:
        print(f"  │     Moderate impact level")
    print()
    ...
    if sens_val >= 0.9:
        print(f"        Financial/healthcare/PII data at risk")
    elif sens_val >= 0.7:
        print(f"        Credentials or proprietary data at risk")
    else:
        print(f"        General data sensitivity")
    print()
