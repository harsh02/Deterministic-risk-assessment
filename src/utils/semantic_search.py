...
def embed_batch(texts: List[str], model: Optional[SentenceTransformer] = None, 
...
def load_ics_stub_index(model: Optional[SentenceTransformer] = None) -> Tuple[Optional[np.ndarray], List[Dict]]:
...
def search_mitre_ttps(query: str,
                      index_dir: Path,
                      top_k: int = 5,
                      min_similarity: float = 0.7,
                      model: Optional[SentenceTransformer] = None,
