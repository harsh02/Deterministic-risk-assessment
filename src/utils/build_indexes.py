...
def build_cve_index(nvd_file: Path, 
                    output_dir: Path,
    ...
    logger.info(f"✅ CVE index built: {len(cve_metadata)} CVEs, {embeddings.shape[1]}-dim embeddings")
    return embeddings, cve_metadata


def build_mitre_index(mitre_file: Path,
                      output_dir: Path,
                      model_name: str = "all-MiniLM-L6-v2"):
    """
    ...
    logger.info(f"✅ MITRE index built: {len(technique_metadata)} techniques, {embeddings.shape[1]}-dim embeddings")
...
def main():
    ...
    parser.add_argument('--max-cves', type=int, default=None,
                       help='Limit CVEs for testing (default: process all)')
    parser.add_argument('--cve-only', action='store_true',
                       help='Build only CVE index')
    parser.add_argument('--mitre-only', action='store_true',
                       help='Build only MITRE index')
